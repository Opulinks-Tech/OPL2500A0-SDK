...
typedef enum
{
    ...
    WIFI_EVENT_STA_CONNECTION_FAILED,   /**< (Deprecated) Connection has failed. For the reason code, please refer to #wifi_reason_code_t. */
...
typedef enum {
    ...
    WIFI_MODE_AP,               /**< (Not Supported) Wi-Fi soft-AP mode */
    WIFI_MODE_MAX
} wifi_mode_t;
...
typedef enum {
    ...
    WIFI_AUTH_WPA3,             /**< authenticate mode : WPA3_SAE */
} wifi_auth_mode_t;
...
typedef enum {
    WIFI_SORT_AP_BY_CHANNEL = 0,    /**< Sort by Channel. */
    WIFI_SORT_AP_BY_SIGNAL,         /**< Sort match AP in scan list by RSSI */
    WIFI_SORT_AP_BY_SECURITY,       /**< (Not Supported) Sort match AP in scan list by security mode */
    WIFI_SORT_AP_BY_SSID,           /**< Sort by SSID. */
}wifi_sort_method_t;
...
/** @brief This enumeration defines wifi power saving mode.
*/
typedef enum {
    WIFI_PS_NONE = 0,               /**< No power saving. */
    WIFI_PS_LIGHT,                  /**< Light power saving. */
    WIFI_PS_DEEP,                   /**< Deep power saving. */
    WIFI_PS_AUTO,                   /**< (Experiment) Auto power saving. */
} wifi_ps_mode_t;
...
/** @brief (Not Supported) Structure describing parameters for a Wi-Fi fast scan */
typedef struct {
...
typedef struct {
    ...
    bool show_hidden;                          /**< (Deprecated) enable to scan AP whose SSID is hidden */
...
typedef struct {
    ...
    uint8_t bssid_present;                    /**< (Deprecated) The BSSID is present if it is set to 1. Otherwise, it is set to 0. */
...
/** @brief (Not Supported) This structure is the Wi-Fi configuration for initialization for Soft-AP mode.
*/
typedef struct {
...
*/
typedef union {
    wifi_ap_config_t ap_config;              /**< (Not Supported) The configurations for the AP. It should be set when the wifi_mode_t is WIFI_MODE_AP . */
...
/**
 * @brief (Deprecated) This structure is the Wi-Fi auto connect with wpa information for save in the flash (FIM).
 */
typedef struct {
    int      proto;
    int      pairwise_cipher;
