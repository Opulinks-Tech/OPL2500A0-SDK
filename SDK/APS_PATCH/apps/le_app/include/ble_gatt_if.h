...
enum
{
	...
	LE_GATT_MSG_READ_MULTI_VAR_CHAR_VAL_CFM,
...
#define LE_GATT_FLAG_EXECUTE_WRITE			0x03
...
typedef struct
{
    ...
    UINT16					len;
	UINT8					*pVal; 
} LE_GATT_MSG_PREPARE_WRITE_IND_T;
...
typedef struct
{
    ...
    UINT8					att_err;
    UINT16					err_hdl;
} LE_GATT_MSG_READ_MULTI_VAR_CHAR_VAL_CFM_T;
...
typedef struct
{
	...
	UINT16					start_hdl;
	UINT16					status;
} LE_GATT_MSG_MULTI_NOTIFY_CFM_T;
...
LE_ERR_STATE LeGattStopCurrentProcedure(UINT16 conn_hdl);
...
LE_ERR_STATE LeGattReadMultipleVariableCharVal(UINT16 conn_hdl, UINT16 count, UINT16 *handle);
...
LE_ERR_STATE LeGttMultiCharValNotify(UINT16 conn_hdl, UINT8 count, LE_ATT_HDL_VAL_T *hdl_val);
