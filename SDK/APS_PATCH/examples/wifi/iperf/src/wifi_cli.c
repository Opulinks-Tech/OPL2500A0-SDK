...
int wifi_cmd_sta(int argc, char** argv)
{
    ...
    if (strncasecmp(argv[1], "-a", 2) == 0) {
        LOGI(TAG, "auto connecting start");
        ret = wifi_auto_connect_start();
        if (ret != 0)
            LOGE(TAG, "auto connecting fail %d", ret);
        return ret;
    }

    LOGI(TAG, "connecting to %s", argv[1]);
    ...
    if (ret != 0)
        LOGE(TAG, "sta connect fail %d", ret);
...
int wifi_cmd_scan(int argc, char** argv)
{
    ...
    LOGI(TAG, "sta scanning");

    wifi_scan_config_t scan_config;
    memset(&scan_config, 0, sizeof(scan_config));

    scan_config.scan_type         = WIFI_SCAN_TYPE_MIX;
    scan_config.sort_method       = WIFI_SORT_AP_BY_SIGNAL;

    ret = wifi_scan_start(&scan_config, NULL);
    if (ret != 0)
        LOGE(TAG, "sta scan fail");
...
int wifi_cmd_query(int argc, char **argv)
{
    ...
    if (bits & CONNECTED_BIT0) {
        ...
        LOGI(TAG, "Netmask:  %s", ip4addr_ntoa(&p_netif->netmask.u_addr.ip4));
        LOGI(TAG, "Gateway:  %s", ip4addr_ntoa(&p_netif->gw.u_addr.ip4));
    }
    else {
...
int wifi_cmd_hl_power(int argc, char **argv)
{
    ...
    if (argc == 2) {
        ...
        if (ret < 0) {
            LOGE(TAG, "set transmit power failed");
            return -1;
        }
    }
