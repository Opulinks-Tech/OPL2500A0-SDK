...
typedef int (*base_mac_addr_src_get_cfg_fp_t) (uint8_t iface, uint8_t* type);
typedef int (*base_mac_addr_src_set_cfg_fp_t) (uint8_t iface, uint8_t type);
typedef int (*base_nvm_mac_addr_src_read_fp_t) (uint16_t id, uint16_t len, void *buf);
typedef int (*base_nvm_mac_addr_src_write_fp_t) (uint16_t id, uint16_t len, void *ptr);
typedef int (*get_rf_power_level_fp_t) (uint8_t *level);
typedef int (*set_rf_power_level_fp_t) (uint8_t level);
typedef int (*get_dhcp_arp_check_fp_t) (void);
typedef int (*get_dhcp_arp_check_from_fim_fp_t) (void);
typedef int (*set_dhcp_arp_check_fp_t) (uint8_t mode);
typedef int (*set_dhcp_interval_retry_times_fp_t)(uint8_t dhcp_mode,uint32_t dhcp_interval,uint8_t dhcp_retry_times);
