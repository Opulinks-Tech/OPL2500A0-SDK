...
os_pthread scrt_sut_task_main_1 = scrt_sut_task_main_1_impl;
os_pthread scrt_sut_task_main_2 = scrt_sut_task_main_2_impl;
os_pthread scrt_sut_task_main_3 = scrt_sut_task_main_3_impl;
os_pthread scrt_sut_task_main_4 = scrt_sut_task_main_4_impl;
scrt_sut_task_delete_fp_t scrt_sut_task_delete = scrt_sut_task_delete_impl;
scrt_sut_task_create_fp_t scrt_sut_task_create = scrt_sut_task_create_impl;
#endif //#ifdef SCRT_CMD_SUT_TASK

void nl_scrt_cmd_impl(char *sCmd);

// external
RET_DATA nl_scrt_cmd_fp_t nl_scrt_cmd = nl_scrt_cmd_impl;
...
#if 1
extern UINT16 LeSmpUtilGenDhkeyApi(UINT8 *pub_key_x, UINT8 *pub_key_y, UINT32 *priv_key, void *dhkey);
extern UINT16 LeSmpUtilGenKeyPairApi(void *pub, UINT32 *priv);
extern UINT16 LeSmpUtilAesCmacApi(const UINT8 *key, const UINT8 *in, UINT16 len, UINT8 *out);

#define LeSmpUtilGenDhkey       LeSmpUtilGenDhkeyApi
#define LeSmpUtilGenKeyPair     LeSmpUtilGenKeyPairApi
#define LeSmpUtilAesCmac        LeSmpUtilAesCmacApi
#endif
...
void nl_scrt_cmd_impl(char *sCmd)
{
    ...
    #endif
    #ifdef SCRT_CMD_SHA
    else if(dwCase == 8) // SHA-1/SHA-224/SHA-256
    {
        //uint8_t u8aOutput[SCRT_SHA_1_OUTPUT_LEN] = {0};
        uint8_t u8aOutput[64] = {0};
        ...
        if(dwParamNum > 4)
        {
            u8Mode = (uint8_t)strtoul((char *)baParam[4], NULL, 10);
        }

        switch(u8Mode)
        {
            case SCRT_TYPE_SHA_1:
                u32DataLen = SCRT_SHA_1_OUTPUT_LEN;
                u32BlkSize = SCRT_SHA_1_STEP_SIZE;
                break;
    
            case SCRT_TYPE_SHA_224:
                u32DataLen = 28; //SCRT_SHA_224_OUTPUT_LEN;
                u32BlkSize = SCRT_SHA_224_STEP_SIZE;
                break;
    
            case SCRT_TYPE_SHA_256:
                u32DataLen = SCRT_SHA_256_OUTPUT_LEN;
                u32BlkSize = SCRT_SHA_256_STEP_SIZE;
                break;

            case SCRT_TYPE_SHA_384:
                u32DataLen = 48; //SCRT_SHA_384_OUTPUT_LEN;
                u32BlkSize = SCRT_SHA_384_STEP_SIZE;
                break;

            case SCRT_TYPE_SHA_512:
                u32DataLen = SCRT_SHA_512_OUTPUT_LEN;
                u32BlkSize = SCRT_SHA_512_STEP_SIZE;
                break;
    
            default:
                tracer_cli(LOG_HIGH_LEVEL, "unknown SHA mode[%u]\n", u8Mode);
                goto done;
        }
        ...
        while(1)
        {
            ...
            if(!nl_scrt_sha(u8Mode, u8Step, u32Total, &(u8aScrtShaData[u32Offset]), u32Input, u8IsInterMac, u8aOutput))
            {
                tracer_cli(LOG_HIGH_LEVEL, "nl_scrt_sha fail\n");
                goto done;
            }
            ...
            tracer_cli(LOG_HIGH_LEVEL, "nl_scrt_sha proc_time: %u us\n", u32Time);
