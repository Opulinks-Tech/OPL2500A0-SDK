...
#define CommandOpcode_Vendor_Specific_Set_AFH_Command                       (CommandOGF_Vendor_Commands << 10 | 0x20)
...
#define SubEventCode_Vendor_Specific_AFH_Channel_Condition_Event	0x21
...
typedef struct
{
    uint8_t ch;             // data_channel, 0~36
    uint8_t err_cnt;        // 0~32
...
typedef struct
{
    ...
    uint8_t blacklist_channels[37];
    uint8_t ridx;
    uint8_t widx;
    uint8_t max_blacklist_size;
    uint8_t dbg_en;
} le_afh_t;
...
// 0x3F.20 Vendor-Specific Set AFH Command
typedef struct {
    uint8_t afh_enable;
    uint8_t interval;           // unit: seconds
    uint8_t dbg_le_info_en;     // [DBG] display le statistics & reset it to evaluate next round
} Vendor_Specific_Set_AFH_Command_Parameters;
...
// 0xFF.18 Vendor-Specific Get Packet Info Statistics Command
typedef struct {
    char *log_buf_addr;
} Vendor_Specific_Get_Packet_Info_Statistics_Command_Return_Parameters;

// 0xFF.21 Vendor-Specific AFH Channel Condition Event
typedef struct 
{
	uint8_t Subevent_Code;
	uint8_t Err_Cnt[37];
} Vendor_Specific_AFH_Channel_Condition_Event_Parameters;
#pragma pack(pop)
...
typedef struct
{
    ...
    // AFH related
    le_afh_t                    afh;                                // for adaptive frequency hopping
