...
void iperf_report_task(void* arg)
{
    ...
    while(1) {
        ...
        if (IS_DUAL_TEST) {
            if (dbg_mode == 0) {
                printf("%4d-%4d sec     %.3f MBytes    %.3f Mbits/sec    %s\n", cur, cur+interval,
                        (double)((iperf_client_ctrl.total_len - iperf_client_ctrl.last_len))/1048576,
                        (double)((iperf_client_ctrl.total_len - iperf_client_ctrl.last_len))/1000000/interval*8,
                        "Sender");
            }
            else {
                printf("%4d-%4d sec     %.3f MBytes    %.3f Mbits/sec    %s    %d\n", cur, cur+interval,
                        (double)((iperf_client_ctrl.total_len - iperf_client_ctrl.last_len))/1048576,
                        (double)((iperf_client_ctrl.total_len - iperf_client_ctrl.last_len))/1000000/interval*8,
                        "Sender",
                        rssi);
            ...
            if (dbg_mode == 0) {
                printf("%4d-%4d sec     %.3f MBytes    %.3f Mbits/sec    %s\n", cur, cur+interval, 
                        (double)((iperf_server_ctrl.total_len - iperf_server_ctrl.last_len))/1048576,
                        (double)((iperf_server_ctrl.total_len - iperf_server_ctrl.last_len))/1000000/interval*8,
                        "Receiver"); 
            }
            else {
                printf("%4d-%4d sec     %.3f MBytes    %.3f Mbits/sec    %s    %d\n", cur, cur+interval, 
            ...
            if (dbg_mode == 0) {
                printf("%4d-%4d sec     %.3f MBytes    %.3f Mbits/sec\n", cur, cur+interval,
                        (double)((p_ctrl->total_len - p_ctrl->last_len))/1048576,
                        (double)((p_ctrl->total_len - p_ctrl->last_len))/1000000/interval*8); 
            }
            else {
                printf("%4d-%4d sec     %.3f MBytes    %.3f Mbits/sec    %d\n", cur, cur+interval,
                        (double)((p_ctrl->total_len - p_ctrl->last_len))/1048576,
                        (double)((p_ctrl->total_len - p_ctrl->last_len))/1000000/interval*8,
                        rssi); 
            }
    ...
    if (IS_DUAL_TEST) {
        printf("%4d-%4d sec     %.3f MBytes    %.3f Mbits/sec    %s\n", 0, cur,
                (double)(iperf_client_ctrl.total_len)/1048576,
                (double)(iperf_client_ctrl.total_len)/1000000/cur*8,
                "Sender total");

        printf("%4d-%4d sec     %.3f MBytes    %.3f Mbits/sec    %s\n", 0, cur,
                (double)(iperf_server_ctrl.total_len)/1048576,
                (double)(iperf_server_ctrl.total_len)/1000000/cur*8,
                "Recever total");
    } else {
        printf("%4d-%4d sec     %.3f MBytes    %.3f Mbits/sec\n", 0, cur, 
                (double)(p_ctrl->total_len)/1048576,
                (double)(p_ctrl->total_len)/1000000/cur*8);
    }
...
static int iperf_report_task_create(void)
{
    ...
    {
        LOGE(TAG, IPERF_REPORT_TASK_NAME"Task create fail \r\n");
        return -1;
    }
    else
    {
...
void iperf_run_udp_server(void *arg)
{
    ...
    iperf_server_ctrl.sockfd = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
    if (iperf_server_ctrl.sockfd < 0) {
        iperf_show_socket_error_reason("udp server create", iperf_server_ctrl.sockfd);
        goto done;
    }

    setsockopt(iperf_server_ctrl.sockfd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt));

    addr.sin_family = AF_INET;
    addr.sin_port = htons(s_iperf_ctrl.sport);
    addr.sin_addr.s_addr = s_iperf_ctrl.sip;
    if (bind(iperf_server_ctrl.sockfd, (struct sockaddr*)&addr, sizeof(addr)) != 0) {
        iperf_show_socket_error_reason("udp server bind", iperf_server_ctrl.sockfd);
        goto done;
    }
    ...
    while (1) {
        FD_ZERO(&read_fd);
        FD_ZERO(&err_fd);
        FD_SET(iperf_server_ctrl.sockfd, &read_fd);
        FD_SET(iperf_server_ctrl.sockfd, &err_fd);
        t.tv_sec = 0;
        t.tv_usec = 300000;
        
        ret = select(iperf_server_ctrl.sockfd + 1, &read_fd, NULL, &err_fd, &t);
        if (ret < 0) {
            goto done;
        }
        ...
        if (FD_ISSET(iperf_server_ctrl.sockfd, &read_fd)) { 
            actual_recv = recvfrom(iperf_server_ctrl.sockfd, buffer, want_recv, 0, (struct sockaddr *)&addr, &addr_len);
            if (actual_recv < 0) {
                iperf_show_socket_error_reason("udp server recv", iperf_server_ctrl.sockfd);
                goto done;
            } else {
                if (!report_created) {
                    ...
                    t.tv_usec = 0;
                    setsockopt(iperf_server_ctrl.sockfd, SOL_SOCKET, SO_RCVTIMEO, &t, sizeof(t));
                    report_created = true;
                }
        ...
        if (FD_ISSET(iperf_server_ctrl.sockfd, &err_fd)) {
            goto done;
        }
    }

done:
    FD_CLR(iperf_server_ctrl.sockfd, &read_fd);
    FD_CLR(iperf_server_ctrl.sockfd, &err_fd);
    close(iperf_server_ctrl.sockfd);
    iperf_server_ctrl.sockfd = -1;
    iperf_server_ctrl.finish = true;
...
void iperf_run_udp_client(void *arg)
{
    ...
    while (!iperf_client_ctrl.finish) {
        ...
        retry = false;
        if (s_iperf_ctrl.random == true) {
            want_send = (rand()%rand_base_len) + sizeof(iperf_udp_pkt_t);
        }
        else if (s_iperf_ctrl.fix_len != 0) {
            want_send = s_iperf_ctrl.fix_len;
        }
...
int iperf_stop(void)
{
    ...
    shutdown(iperf_server_ctrl.sockfd, SHUT_RDWR);

    cfg_mode |= IPERF_FLAG_ABORT;

    printf("\n");
    LOGI(TAG, "Aborted iperf");
