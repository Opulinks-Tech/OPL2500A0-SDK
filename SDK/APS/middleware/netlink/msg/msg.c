...
int tracer_msg_rtl_sim(uint8_t bType, uint8_t bLevel, char *sFmt, ...);
int tracer_drct_printf_rtl_sim(const char *sFmt, ...);
#endif /* __RTL_SIMULATION__ */

// internal
uint8_t g_bTracerLogMode;
int g_iTracerPriority;
uint32_t g_dwTracerStackSize;
uint32_t g_dwTracerQueueNum;
uint32_t g_dwTracerQueueSize;
uint8_t g_bTracerIntTaskNum = TRACER_INT_TASK_NUM_MAX;
uint8_t g_bTracerExtTaskNum = TRACER_EXT_TASK_NUM_MAX;
uint8_t g_bTracerExtTaskDefLevel;
T_TracerTaskInfo *g_ptTracerDefIntTaskInfo = (T_TracerTaskInfo *)g_taTracerDefIntTaskInfoBody;
T_TracerTaskInfo *g_ptTracerIntTaskInfo = g_taTracerIntTaskInfoBody;
T_TracerTaskInfo *g_ptTracerExtTaskInfo = g_taTracerExtTaskInfoBody;
uint8_t g_bTracerNameDisplay;

T_TracerProcFp tracer_proc = tracer_proc_impl;
T_TracerMsgFreeFp tracer_msg_free = tracer_msg_free_impl;
T_TracerOptEntryAddFp tracer_opt_entry_add = tracer_opt_entry_add_impl;
T_TracerNameGetFp tracer_task_name_get = tracer_task_name_get_impl;
T_TracerTaskMainFp tracer_task_main = tracer_task_main_impl;
T_TracerHandleGetFp tracer_task_handle_get = tracer_task_handle_get_impl;
T_TracerLevelGetFp tracer_level_get = tracer_level_get_impl;
T_TracerCommonFp tracer_load = tracer_load_impl;
T_TracerTaskCfgSaveFp tracer_cfg_save = tracer_cfg_save_impl;
T_TracerTaskInfoSaveFp tracer_int_task_info_save = tracer_int_task_info_save_impl;
T_TracerTaskInfoSaveFp tracer_ext_task_info_save = tracer_ext_task_info_save_impl;
T_TracerCommonFp tracer_cfg_reset = tracer_cfg_reset_impl;
T_TracerCommonFp tracer_int_task_reset = tracer_int_task_reset_impl;
T_TracerCommonFp tracer_ext_task_reset = tracer_ext_task_reset_impl;
T_TracerStringSendFp USART_SendString;
T_TracerStringSendFp USART_SendStringII;

// external
T_TracerCommonFp tracer_init = tracer_init_impl;
T_TracerLogLevelSetFp tracer_log_level_set = tracer_log_level_set_impl;
T_TracerOptSetFp tracer_log_mode_set = tracer_log_mode_set_impl;
T_TracerOptGetFp tracer_log_mode_get = tracer_log_mode_get_impl;
T_TracerPrioritySetFp tracer_priority_set = tracer_priority_set_impl;
T_TracerCommonFp tracer_dump = tracer_dump_impl;
T_TracerNameDisplayFp tracer_name_display = tracer_name_display_impl;

#if defined(__RTL_SIMULATION__)
T_TracerPrintfFp tracer_drct_printf = tracer_drct_printf_rtl_sim;
T_TracerMsgFp tracer_msg = tracer_msg_rtl_sim;
#else /* FPGA and Chip */
T_TracerPrintfFp tracer_drct_printf = tracer_drct_printf_impl;
T_TracerMsgFp tracer_msg = tracer_msg_impl;
#endif /* __RTL_SIMULATION__ */
T_TracerTaskInfoGetFp tracer_task_info_get = tracer_task_info_get_impl;
T_TracerDefLevelFp tracer_def_level_set = tracer_def_level_set_impl;
T_TracerCmdFp tracer_cmd = tracer_cmd_impl;
...
void tracer_proc_impl(char *sString)
{
    ...
    {
        if(*pt == '\n') {
            Hal_Uart_DataSend(UART_IDX_DBG, '\r');
        }
        Hal_Uart_DataSend(UART_IDX_DBG, *pt++);
    }
#endif
...
int fputc(int ch, FILE *f)
{
    ...
    Hal_Uart_DataSend(UART_IDX_DBG, ch);
    return ch;
#endif
}
